...
set /p TAR_FILE="/tmp/janus.zip"
set /p TAR_FILE_SIG="/tmp/janus.zip.sig"
set /p ISAAC_GPG_FILE="/tmp/isaac-gpg.txt"
set /p ISAAC_GPG_URL="https://raw.githubusercontent.com/ethereumproject/volunteer/master/Volunteer-Public-Keys/isaac.ardis%40gmail.com"
set /p DOWNLOAD_URL="https://github.com/whilei/janus/releases/download"
test -z "%TMPDIR%" && set /p TMPDIR="$(mktemp -d)"
...
download() {
  test -z "%VERSION%" && set /p VERSION="$(last_version)"
  test -z "%VERSION%" && {
